/**
 * Renders a trapezoid shape on the canvas.
 */
import { useRef } from 'react'
import { Group, Line, Transformer } from 'react-konva'
import type Konva from 'konva'
import type { TrapezoidObject } from '../../../types/objects'
...
interface TrapezoidShapeProps extends BaseShapeProps {
  obj: TrapezoidObject
}

function TrapezoidShapeInner({
  obj,
  viewportRef,
  canEdit,
  selected,
  ...
  const handleTransformEnd = () => {
    ...
    ;(node.findOne('Line') as Konva.Line)?.points([sk, 0, newWidth - sk, 0, newWidth, newHeight, 0, newHeight])
...
export const TrapezoidShape = React.memo(TrapezoidShapeInner, areShapePropsEqual)
